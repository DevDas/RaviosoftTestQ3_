...
void AGun::ShootWithLineTrace()
{
	...
	if (MyOwner)
	{
		...
		if (GetWorld()->LineTraceSingleByChannel(Hit, EyeLocation, TraceEnd, COLLISION_GUN, QueryParams)) // From Pawn_BP->Camera->EyeHeight/ Changed It
		{
			...
			UGameplayStatics::SpawnEmitterAtLocation(GetWorld(), ImpactBlast, Hit.Location, ShotDirection.Rotation());
			UGameplayStatics::SpawnEmitterAttached(MuzzleBlast, GunMesh, FName("Projectile"));
...
void AGun::ShootWithProjectile()
{
	...
	if (MyOwner)
	{
		...
		FRotator Rotation = (TraceEnd - GunMesh->GetSocketLocation(FName("Projectile"))).Rotation(); // Rotating The Projectile To Crosshair

		AProjectileType* Projectile = GetWorld()->SpawnActor<AProjectileType>(ProjectileBlueprint, GunMesh->GetSocketLocation(FName("Projectile")), Rotation);
		if (Projectile)
		{
